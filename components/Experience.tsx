        ...
        {experiences.map((exp, index) => (
          ...
          >
            <div className="flex gap-4">
              <div className="text-4xl group-hover:scale-125 group-hover:animate-wiggle transition-transform duration-300 flex-shrink-0 animate-bounce-light" style={{ animationDelay: `${index * 100 + 100}ms` }}>{exp.icon}</div>
              <div className="flex-1 min-w-0">
                <h3 className="text-lg font-bold text-blue-400 mb-2 group-hover:text-blue-300 transition-colors animate-fade-in-up">{exp.title}</h3>
                <p className="text-blue-500 text-sm font-semibold mb-2 group-hover:text-blue-400 transition-colors animate-fade-in-up animate-delay-100">{exp.period}</p>
                <p className="text-gray-400 group-hover:text-gray-300 transition-colors leading-relaxed animate-fade-in-up animate-delay-200">{exp.description}</p>
              </div>
            </div>
          </div>
        ))}
